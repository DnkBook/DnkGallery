...
1. In `DnkGallery.Presentation.csproj`, replace the `<TargetFrameworks ... />` elements with the `<TargetFrameworks ... />` elements from `DnkGallery.csproj`; both projects should have the same target frameworks
...
2. To quickly try out the C# Markup 2 `Example`, add it to your navigation.
   ...
   At the top, add:
   ```csharp
   using CsMarkup2Example = DnkGallery.Presentation.Example;
   ```
...
3. **Rebuild** the `DnkGallery.Windows` project and run it
...
You can use `New-View.ps1` (located in the `DnkGallery.Presentation` project folder) to quickly add new views - including (view)model, if applicable. See `New-View.ps1` for tips and parameters.
